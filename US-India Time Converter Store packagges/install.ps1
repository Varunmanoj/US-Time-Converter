...
Write-Host "Installing US-India Time Converter..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "US-India Time Converter.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "US-India Time Converter"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named US-India Time Converter. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching US-India Time Converter..."
